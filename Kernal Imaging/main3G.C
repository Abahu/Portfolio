...
/*
	Color red(200, 150, 255, 100, 100);
	Color nored(200, 150, 100, 255, 100);
	TBConcat tb;
	Blur mirror;

	tb.SetInput(red.GetOutput());
	tb.SetInput2(nored.GetOutput());
	mirror.SetInput(reader.GetOutput());

	PNMwriter writer;
	writer.SetInput(mirror.GetOutput());
	mirror.GetOutput()->Update();
	writer.Write(argv[2]);
*/
}
